...
interface ProjectApprovedParams {
  project: {
    id: number;
    title: string;
    description: string;
...
export class ProjectApprovedStrategy implements IEmailStrategy {
  build(params: any, template: string) {
    const parameters = params as ProjectApprovedParams;
    
    const projectUrl = `${parameters.feUri}/project/${parameters.projectType}/view/${parameters.project.id}`;

    return {
      subject: 'Publicacion aprobada - Assets 10',
      html: template
