...
export class ProjectClosedStrategy implements IEmailStrategy {
  build(params: any, template: string) {
    const parameters = params as ProjectClosedParams;
    
    const projectUrl = `${parameters.feUri}/project/${parameters.projectType}/view/${parameters.project.id}`;

    return {
      subject: 'Trato Cerrado - Assets 10',
      html: template
